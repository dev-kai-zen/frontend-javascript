...
function formatJsonPreview(value, maxLen = 72) {
...
export default function UserLogsPage() {
  ...
  /** Server does not return total count; when the page is full, assume at least one more page exists. */
  const paginationCount = useMemo(() => {
    const loadedThrough = pageIndex * pageSize + rows.length;
    const hasMore = rows.length === pageSize;
    return hasMore ? loadedThrough + pageSize : loadedThrough;
  ...
  const stats = useMemo(() => {
    ...
    const distinctUsers = new Set(
      rows.map((r) => r.user_id).filter((id) => id != null),
    ).size;
